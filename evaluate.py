...
def main():
    ...
    if cfg[mode].dump_dir is not None:
        cfg[mode].dump_dir = os.path.join(
            cfg[mode].dump_dir, cfg.workspace.strip("/").split("/")[-1]
        )
    else:
        cfg[mode].dump_dir = ""
    print(cfg)
        ...
        if len(pt_list) > 1:
            ...
            for pt in pt_list:
                if cfg[mode].pretrained_best_type in pt:
                    pt_to_be_loaded = pt
                    break
        cfg.pretrained_model_file = os.path.join(
            cfg.workspace, "checkpoints", pt_to_be_loaded
        )
    ...
    for example_idx, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        step_out = do_eval_step(
            batch, head_outputs, loss_dict, eval_kwargs, dump_dir=cfg[mode].dump_dir
        )
    ...
    for fp in tqdm(file_paths):
        with open(fp, "r") as f:
            ...
            while True:
                ...
                cv2.rectangle(
                    img, tuple(box_father[:2]), tuple(box_father[2:]), color_box, 2
                )
                center_father = (
                    (box_father[0] + box_father[2]) // 2,
                    (box_father[1] + box_father[3]) // 2,
                )
                box_son = blk_coord_dict[son_id]
                cv2.rectangle(img, tuple(box_son[:2]), tuple(box_son[2:]), color_box, 2)
                center_son = (
                    (box_son[0] + box_son[2]) // 2,
                    (box_son[1] + box_son[3]) // 2,
                )
                # link
                cv2.arrowedLine(
                    img,
